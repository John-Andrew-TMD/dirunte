<template>
  <div class="container">
    ...
    <el-table height="550px" border size="small" :data="data" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="轮播图图片" width="110" align="center">
      ...
      <el-table-column prop="title" label="口号" width="200"></el-table-column>
  ...
  <!--添加轮播图-->
  <el-dialog title="添加轮播图" v-model="centerDialogVisible">
    <el-form label-width="100px" :model="registerForm">
      ...
      <el-form-item label="轮播图介绍" prop="introduction">
...
export default defineComponent({
  ...
  setup() {
    ...
    async function getData() {
      tableData.value = [];
      tempDate.value = [];
      const result = (await HttpManager.getSwiperList()) as ResponseBody;
    ...
    function uploadUrl(id) {
      return HttpManager.attachImageUrl(`/swiperList/img/update?id=${id}`);
    }
    // 更新图片
    ...
    function goContentPage(id) {
      ...
      routerManager(RouterName.ListSwiper, { path: RouterName.ListSwiper, query: { id } });
    }
    ...
    async function saveEdit() {
      ...
      const result = (await HttpManager.updateSwiperListMsg(params)) as ResponseBody;
      (proxy as any).$message({
        message: result.message,
        type: result.type,
      });
    ...
    async function confirm() {
      const result = await HttpManager.deleteSwiperList(idx.value) as ResponseBody;
      (proxy as any).$message({
        message: result.message,
        type: result.type,
      });
